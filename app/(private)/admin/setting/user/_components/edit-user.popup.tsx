...
const EditUserPopup = ({ open, data, onOpenChange }: IAddRolePopupProps) => {
  ...
  useEffect(() => {
    ...
    for (let idx = 0; idx < data.roles.length; idx++) {
        const item = data.roles[idx];
        console.log(item);
        arrRoleIds.push(item.id);
    }

    setSelectedRole(arrRoleIds)
  ...
  const onClickSubmitForm = async () => {
    try {
      ...
      let res = await userApiRequest.sUpdateUser(parameter);
